...
function [LNS_L, LNS_Gam, replaced_rows] = local_apply_south_condition(LNS_L, LNS_Gam, idx, i, layout, bc_name, cN)
%LOCAL_APPLY_SOUTH_CONDITION Apply symmetry/wall rules on the south edge.

    row_sets = local_get_south_row_sets(layout, bc_name);
...
function row_sets = local_get_south_row_sets(layout, bc_name)
    ...
    switch layout.name
        case 'primitive5_u_v_w_T_p'
            if strcmp(bc_name, 'symmetry')
                ...
                row_sets.dirichlet = [layout.components.u, layout.components.v, layout.components.w, layout.components.T];
